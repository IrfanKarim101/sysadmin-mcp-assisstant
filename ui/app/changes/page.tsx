...
type Change = {
  ...
  evidence: { stage: string; status: string; remote_mutation: boolean; snapshots?: { reference: string; snapshot_type: string; checksum: string; expires_at: string; verified: boolean }[]; post_rollback_check?: string; fleet_rollout?: { canary: string; concurrency: number; timeout_seconds: number; circuit_breaker: string; failed_host?: string | null; skipped_hosts?: string[]; waves: string[][]; hosts: { host: string; wave: number; state: string; lock: string }[] }; [key: string]: unknown }[];
  approval_token?: string;
  approval_owner?: string | null; approval_expires_at?: string | null; rollback_available: boolean;
};
      ...
      <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
        ...
        <div className="space-y-4">{items.length === 0 && <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">No transactions in this session.</div>}{items.map((item) => <article key={item.id} className="rounded-2xl border bg-card p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
            ...
            {item.approval_expires_at && <span className="text-amber-200">Approved by {item.approval_owner} · expires {new Date(item.approval_expires_at).toLocaleString()}</span>}
          ...
          {item.evidence.at(-1)?.fleet_rollout && <div className={`mt-3 rounded-lg border p-3 text-xs ${item.evidence.at(-1)!.fleet_rollout!.circuit_breaker === 'open' ? 'border-red-400/30 bg-red-400/5' : ''}`}><p className="font-medium">Fleet rollout · canary {item.evidence.at(-1)!.fleet_rollout!.canary}</p><p className={`mt-1 ${item.evidence.at(-1)!.fleet_rollout!.circuit_breaker === 'open' ? 'text-red-200' : 'text-muted-foreground'}`}>Concurrency {item.evidence.at(-1)!.fleet_rollout!.concurrency} · timeout {item.evidence.at(-1)!.fleet_rollout!.timeout_seconds}s · circuit {item.evidence.at(-1)!.fleet_rollout!.circuit_breaker}{item.evidence.at(-1)!.fleet_rollout!.failed_host ? ` · failed host ${item.evidence.at(-1)!.fleet_rollout!.failed_host}` : ''}</p><div className="mt-2 flex flex-wrap gap-2">{item.evidence.at(-1)!.fleet_rollout!.hosts.map((host) => <span key={host.host} className={`rounded-full border px-2 py-1 ${host.state === 'simulated_failed' ? 'border-red-400/30 text-red-200' : host.state === 'skipped_circuit_open' ? 'border-amber-400/30 text-amber-200' : ''}`}>wave {host.wave + 1} · {host.host} · {host.state}</span>)}</div></div>}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {item.state === 'approved' && <Button disabled={busy || !tokens[item.id]} onClick={() => call(`/api/changes/${item.id}/start`, { approval_token: tokens[item.id] })}>Start with backup</Button>}
            {['backed_up','applying','validating','activating'].includes(item.state) && <>{item.state === 'validating' && item.preview?.some((x) => x.service?.material_approval_required) && <Input className="max-w-56" type="password" placeholder="Confirm material activation" value={activationPassword} onChange={(e) => setActivationPassword(e.target.value)}/>}<Button disabled={busy || (item.state === 'validating' && !!item.preview?.some((x) => x.service?.material_approval_required) && !activationPassword)} onClick={async () => { await call(`/api/changes/${item.id}/advance`, { ...(activationPassword ? { activation_password: activationPassword } : {}) }); setActivationPassword(''); }}>Advance one stage</Button></>}
