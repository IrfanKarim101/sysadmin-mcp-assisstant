...
import { GitPullRequestDraft, LoaderCircle, ShieldAlert } from 'lucide-react';
...
type Change = {
  id: string; title: string; state: string; actions: Action[]; plan_hash: string;
  ...
  evidence: { stage: string; status: string; remote_mutation: boolean; snapshots?: { reference: string; snapshot_type: string; checksum: string; expires_at: string; verified: boolean }[]; post_rollback_check?: string }[];
...
export default function ChangesPage() {
  ...
  async function load() {
    const [hostResponse, changeResponse, policyResponse, packageResponse] = await Promise.all([apiFetch('/api/hosts'), apiFetch('/api/changes'), apiFetch('/api/managed-files/policies'), apiFetch('/api/packages/policies')]);
    if (!hostResponse.ok || !changeResponse.ok || !policyResponse.ok || !packageResponse.ok) throw new Error('Could not load change workspace.');
  ...
  useEffect(() => { void load().catch((reason) => setError(reason instanceof Error ? reason.message : 'API unavailable.')); }, []);
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="flex min-h-16 items-center border-b px-6 py-3"><div><h1 className="font-semibold">Change transactions</h1><p className="text-xs text-muted-foreground">Human-approved Phase 16 workflow</p></div><AppNav /></header>
      ...
      <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
        ...
        <div className="space-y-4">{items.length === 0 && <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">No transactions in this session.</div>}{items.map((item) => <article key={item.id} className="rounded-2xl border bg-card p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {item.state === 'approved' && <Button disabled={busy || !tokens[item.id]} onClick={() => call(`/api/changes/${item.id}/simulate`, { approval_token: tokens[item.id] })}>Run simulation</Button>}
